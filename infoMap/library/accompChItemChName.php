<?PHP
    ...
    $filter = "extra_data <> 'notify_SET' AND extra_data <> 'notify_ACC' AND in_use = 1 AND (en_name LIKE '%$q%' OR in_group LIKE '%$q%') AND cat_id = '32'";
    ...
    while ($records[] = dbFetchAssoc($results)) { 
          ...
          //if(!isJSON($dose_option)){
              array_push($arrRtn, array("id"=>$record_id, "label"=>$ch_name, "value"=>$ch_name,"pinyin_name"=>$product_name,"bplus_id"=>$bplus_id,"cost"=>$purchasing_price,"dose"=>$dose_option));  
          //}
    }

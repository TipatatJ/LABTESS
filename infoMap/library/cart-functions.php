...
$result = dbQuery($sql, 'error here3<br>'.$sql);*/
...
function addToCart()
{
	...
	if (dbNumRows($result) == 0) {
		...
		$sql = "INSERT INTO tbl_cart (pd_id, ct_qty, ct_session_id, ct_date, ct_dose_option, ct_discounted_price)
				VALUES ($productId, 1, '$sid', NOW(),'$dose_option','$pd_price')";
		$result = dbQuery($sql);
	} else {
...
function getCartContent()
{
	...
	$sql = "SELECT ct_id, ct.pd_id, ct_session_id, ct_qty, ct_dose_option, ct_discounted_price, pd_name, pd_price, pd_thumbnail, pd.cat_id, pd.pd_qty, pd.class, pd.subclass, pd.in_group, pd.convertable_to, pd.bplus_id, pd.unit, pd.in_use, pd.note, pd.dose_option, pd.std_set,  pd.preorder_amount, pd.extra_data, pd.purchasing_price, sale_acc
	...
	while ($row = dbFetchAssoc($result)) {
        ...
        //echo $row['sale_acc'].'<hr>';
        
        //$row['ct_dose_option'] = str_replace('"','\"',$row['ct_dose_option']);
		$cartContent[] = $row;	//นำข้อมูลจากฐานข้อมูลเก็บเข้า array
	}
    
    //echo 'getCartContent---v';
    //echo var_dump($cartContent).'<hr>';

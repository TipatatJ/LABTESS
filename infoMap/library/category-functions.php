...
{
	$sql = "SELECT cat_id, cat_name, cat_image, cat_url, auth_role
	        FROM tbl_category
...
{
    //Present Cat to Left Nav bar
    $sql = "SELECT cat_id, cat_parent_id, cat_name, cat_image, cat_description, cat_tools, cat_url, auth_role
	        FROM tbl_category
...
function getMainCategoryForNev()
{
...
function getChildCategoryForNev($parrentCatId)
{
	$sql = "SELECT cat_id, cat_name, cat_image
	        FROM tbl_category
			WHERE cat_parent_id = $parrentCatId
			ORDER BY cat_id";
    $result = dbQuery($sql);
...
function fetchCategoriesForNev()
{
	$catString ='';
    $mainCat = getMainCategoryForNev();
    $m     = count($mainCat);

    for($i = 0; $i < $m; $i++){
    	extract($mainCat[$i]);
    	$catString .= '<li><a class="category-group-item" id="'.$mainCat[$i]['id'].'" style="border:none!important;">'.$mainCat[$i]['name'].'</a></li>';
    	$childCat = getChildCategoryForNev($mainCat[$i]['id']);
    	$n     = count($childCat);
    	for($j = 0; $j < $n; $j++){
    		$catString .= '<li class="navbar-collapse dropdown-submenu"><a class="category-group-item" id="'.$childCat[$j]['id'].'" style="border:none!important;">&raquo;&nbsp;'.$childCat[$j]['name'].'</a></li>';
    	}
    }
...
//echo 'end of category-functions.php<br>';
?>
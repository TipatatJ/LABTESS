<?PHP
    ...
    $filter = "extra_data <> 'notify_SET' AND extra_data <> 'notify_ACC' AND in_use = 1 AND (en_name LIKE '%$q%' OR in_group LIKE '%$q%')";
    ...
    while ($records[] = dbFetchAssoc($results)) { 
          ...
          array_push($arrRtn, array("id"=>$record_id, "label"=>$en_name, "value"=>$en_name,"product_name"=>$product_name,"bplus_id"=>$bplus_id,"cost"=>$purchasing_price,"dose"=>$dose_option));  
